...
**Backdrop — `components/brand/Atmosphere.tsx`.** Server-rendered; the only JavaScript is a pointer listener. Layers back to front: navy base `#001738 → #000D24`; two drifting aurora blobs (CUT blue top-left, gold bottom-right, 70 px blur, 26–34 s alternate drift); **the ballroom of light** (`Ballroom.tsx`, below); the CUT watermark symbol at 5 % with the logotype masked out; 28 rising gold light particles on a deterministic field; SVG film grain at 35 % overlay; a radial vignette. An optional photograph can sit under the aurora at 38 % opacity. The whole scene breathes with a 40 s Ken Burns drift (`.kenburns`, scale 1 → 1.06) and answers the pointer through three parallax depths (`.px-far` 6 px, `.px-mid` 14 px, `.px-near` 28 px, driven by `Parallax.tsx`; off on touch devices). `intensity` scales the blobs, scene, watermark and particles (1 on hero pages, 0.6 on the pass page). Reduced motion freezes everything.

**The ballroom of light — `components/brand/Ballroom.tsx`.** No photograph is needed to put the guest in the room. Three rows of candle bokeh recede in perspective: 26 small, nearly sharp lights at the back (48–54 % height), 14 warmer mid lights (58–66 %), 7 large out-of-focus lights in the foreground (72–90 %, up to 96 px, 22 px blur), each flickering on its own 3–7 s cycle. A chandelier of 46 twinkling points with a warm halo sits at 56 % / 22 %. Two stage beams sway ±6° over 26–31 s. A horizon haze dissolves the far tables and a floor glow warms the bottom third. All positions come from a seeded generator so server and client agree. Palette is gold `#FBB927`, warm `#F6A623` and pale `#FFD66B` only, so the scene reads as CUT gold on CUT navy and never as a stock photograph.

**Type on the cinematic layer.** Headline `clamp(3.5rem, 6.2–7.5vw, 6.75–7.25rem)`, Barlow Condensed 700, line-height 0.92, one line white and one line `.text-gold-metallic` (a warm gold gradient clipped to the text with a slow 9 s sheen). Eyebrows use `.eyebrow-on-dark` at 0.18 em tracking. Body copy `white/70` at 18–20 px. Copy is short, declarative, and about giving: the house line is **"Where CUT gathers. Where CUT gives."** It speaks for every event and every gift, never for one night.
...
**What an outward-facing page may say.** The sign-in page and the landing page are public. They show the featured event as a poster would — `EventCard.tsx`: title, date, venue, "Happening now" or "Next on the calendar", and "By invitation" — and nothing that is counted or priced. No amount raised, no arrivals, no lots, no bids, no ticker of money. Data comes from `getPublicEvent()` in `lib/public/showcase.ts`, which selects only those columns and returns null on any failure so a public page can never 500 because the database is asleep. The foot of the page is still, not scrolling: Institutional Advancement's four pillars (Fundraising · Development · Alumni Relations · Stewardship) with gold diamonds, the university name, and the motto.

**Live strip — `components/brand/LiveStrip.tsx`.** The signed-in and in-room counterpart: the featured event with three counting figures (raised so far in metallic gold ZAR, guests arrived, lots open) and a `Marquee` ticker of the last anonymised bids: "◆ Lot 3 · title · R5 500 · Bidder 017". Figures animate on entry with `CountUp` (mode `zar` or `int`; never pass a function across the client boundary). Data comes from `getShowcase()`. **Use it only on authenticated surfaces (dashboard, console) and on the projection, which is by definition inside the room.** Never on `/` or `/login`.
