...
- [ ] **Webhooks need a public URL.** Resend, Meta and Yoco call back over the internet, so `POST /api/webhooks/resend`, `GET/POST /api/webhooks/whatsapp` and `POST /api/webhooks/yoco` must be reachable: the Vercel deployment in the normal case, or a tunnel (`cloudflared tunnel --url http://localhost:3000`) while developing. For WhatsApp, register the callback URL and `WHATSAPP_VERIFY_TOKEN` under the Meta app's WhatsApp → Configuration and subscribe to the `messages` field; Meta verifies with a GET and then signs every POST with the app secret (`WHATSAPP_APP_SECRET`). Nothing else is blocked without the webhooks: statuses simply stay at `sent`.
