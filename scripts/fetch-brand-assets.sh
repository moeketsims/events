...
{
  echo "# Brand asset sources"
  echo
  echo "Fetched $(date -u +%Y-%m-%dT%H:%M:%SZ) from CUT's Corporate Identity page https://www.cut.ac.za/ci."
  echo "These files are the property of Central University of Technology, Free State and are used here"
  echo "under the institution's own brand rules for an internal CUT system. Do not redistribute."
  echo
  echo "| File | Source URL | Bytes |"
  echo "|---|---|---|"
} > "$OUT/SOURCES.md"

for name in $(printf '%s\n' "${!FILES[@]}" | sort); do
  url="$BASE/${FILES[$name]}"
  if curl -fsSL --retry 3 --max-time 60 "$url" -o "$OUT/$name"; then
    size=$(stat -c %s "$OUT/$name" 2>/dev/null || stat -f %z "$OUT/$name")
    echo "| $name | $url | $size |" >> "$OUT/SOURCES.md"
    echo "ok   $name ($size bytes)"
  else
    echo "| $name | $url | FAILED |" >> "$OUT/SOURCES.md"
    echo "FAIL $name"
  fi
done
